...
import com.tencent.qcloud.suixinbo.giftconstant.GiftConstant;
...
//    private static int postion;
    ...
    private Queue<ChatEntity> cache = new LinkedList<>();
    ...
    // 收到礼物，等待显示动画
    public void showGiftAnimation(final ChatEntity message) {
        cache.add(message);
        checkAndStart();
    }
    ...
    private void updateView(final ChatEntity message, ViewGroup root) {
        // senderName
        TextView audienceNameText = (TextView) root.findViewById(R.id.audience_name);
        if (message.getSenderName() != null) {
            audienceNameText.setText(message.getSenderName());
        } else {
            audienceNameText.setText(DemoCache.getUserInfo() == null ? DemoCache.getAccount() : DemoCache.getUserInfo().getNickName());
        }

        // gift name & image
//        GiftAttachment attachment = (GiftAttachment)message.getCustomStr();
        TextView giftText = (TextView) root.findViewById(R.id.gift_name);
        giftText.setText(GiftConstant.titles[LiveActivity.pos]);

        ImageView giftImage = (ImageView) root.findViewById(R.id.gift_image);
        giftImage.setImageResource(GiftConstant.images[LiveActivity.pos]);
