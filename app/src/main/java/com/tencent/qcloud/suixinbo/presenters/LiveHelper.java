...
//                        mLiveView.showVideoView(LOCAL, CurLiveInfo.getHostID());
    ...
    public void sendGiftCMessage(int cmd, String Param, final TIMMessage msg, final String sendId) {
...
//        TIMMessage msg = new TIMMessage();
        ...
        mC2CConversation.sendMessage(msg, new TIMValueCallBack<TIMMessage>() {
            ...
            public void onSuccess(TIMMessage timMessage) {
                ...
                for (int j = 0; j < timMessage.getElementCount(); j++) {
                    String send_name_actual = timMessage.getConversation().getIdentifer();
                    TIMCustomElem elem = (TIMCustomElem) timMessage.getElement(0);
                    byte[] bytes = elem.getData();
                    String send_name = new String(bytes);
                    String actionParam = JsonUtil.getFieldValue(send_name,"actionParam");
                    LogUtil.e("--------",actionParam);
                    LogUtil.e("--------",send_name);
//                        if (timMessage.isSelf()) {
//                            LogUtil.e("======","1");
////                            handleGiftMessage(elem.getDesc());
//                            handleGiftMessage(msg.getSender());
//                        } else {
//                            TIMUserProfile sendUser = timMessage.getSenderProfile();
//                            String name;
//                            if (sendUser != null) {
...
//                            }
//                        }
                    handleGiftMessage(send_name_actual);
                }
            }
        });
     ...
     * @param name
     */
    public void handleGiftMessage(String name) {

//        Toast.makeText(mContext, "" + textElem.getText(), Toast.LENGTH_SHORT).show();
        LogUtil.e("send_", "5");
        mLiveView.showGiftMessage(name);
    }
...
//
            ...
            for (int j = 0; j < currMsg.getElementCount(); j++) {
                ...
                String faceUrl = currMsg.getSenderProfile().getFaceUrl();
                LogUtil.e("faceUrl",faceUrl);
        ...
        try {
            ...
            switch (action) {
                ...
                case Constants.AVIMCMD_Praise:
                    ...
                    break;
                case Constants.AVICMD_Gift:
                    mLiveView.showGiftMessage(nickname);
